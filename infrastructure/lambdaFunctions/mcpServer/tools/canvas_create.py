...
def canvas_create(channel: str, markdown: str, title: str = "") -> dict:
    ...
    initial content (headings with #, lists with -, checklists with - [ ]).
    Optionally set `title`, the name shown on the canvas tab; without it the
    canvas is "Untitled". A channel can only have one canvas: if it already has
    one, this returns an error pointing you at canvas_edit. The canvas is
    authored as you (the calling agent).
    """
    try:
        ...
        canvas_id = create_channel_canvas(channel, markdown, title)
